...
- The character list has switchable card layouts (compact / portrait / gallery); each layout keeps its own card size and font-size (`CharacterLayout` = selected type + `Map<CharacterLayoutType, CharacterLayoutSettings>`, which holds `cardWidth`/`cardHeight`/`fontSize`; defaults: compact 380×120, portrait 380×160, gallery 380×220). Dialog from the list app bar has a single **Card** section (width/height/font sliders); persisted with `shared_preferences` per-type keys `charW_`/`charH_`/`charFont_` + `charLayoutType`; see `character_layout.dart` (`characterLayoutProvider`). The photo box is always **derived from the card** by the pure function `characterCardMetrics(s, type)` — in compact/portrait the photo is a **height-driven square** (`_sidePhotoSide` = `min(cardH − 12, cardW · 0.5)`, so width never stretches it or pushes the text away; extra width just becomes empty space on the right), in gallery the full card width minus a 32px name strip — so the image is always fully visible and never larger than the card. Cards are `SizedBox`es in a `Wrap`; photos use a fixed `SizedBox` (`compactPhotoBox`/`portraitPhotoBox`/`galleryPhotoBox` keys) with `CoverImage(fill: true, fillFit: BoxFit.contain)` — always contained, never cropped. Compact and portrait cards both show name (age) / profession / description lines (compact description wraps up to 2 lines, portrait up to 3) and are aligned top-left: the photo is flush with the card's top-left corner (`padding: fromLTRB(0, 0, 6, 6)`, `CoverImage(alignment: Alignment.topLeft)`) and the text column starts at the top. Card text never uses `TextOverflow.ellipsis` — lines break up to each field's `maxLines` and any overflow is clipped by the card's `Clip.antiAlias`. The dialog sliders are a custom `_SteppedSlider` (gesture + `Align`/`FractionallySizedBox`, stepped via round-to-step) — **not** Material `Slider`, which wraps its value indicator in an `OverlayPortal` and floods the Windows log with `Failed to update ui::AXTree` (flutter/flutter#190357). It must stay LayoutBuilder-free: `AlertDialog` queries intrinsic dimensions of content and `LayoutBuilder` throws there.
