...
- The character list has switchable card layouts (compact / portrait / gallery); each layout keeps its own width/height, font-size and whole-image setting (`CharacterLayout` = selected type + `Map<CharacterLayoutType, CharacterLayoutSettings>`; defaults: compact 380×120, portrait 380×160, gallery 380×220 with whole-image off). Dialog from the list app bar, persisted with `shared_preferences` (per-type keys `charW_`/`charH_`/`charFont_`/`charWhole_`); see `character_layout.dart` (`characterLayoutProvider`). Cards are laid out with a `Wrap` of fixed `SizedBox`es sized from `settings.cardWidth`/`cardHeight`. `wholeImage` true → whole photo shown never cropped (compact/portrait via `CoverImage(fixedHeight:)` aspect-contain, gallery via `CoverImage(fill: true, fillFit: contain)`); false → photo fills/crops the photo box (`CoverImage(fill: true)` cover, photo width = `cardWidth * 0.36`/`0.42`).
