...
- The character list has switchable card layouts (compact / portrait / gallery); each layout keeps its own card and photo sizes, font-size, whole-image and aspect-lock setting (`CharacterLayout` = selected type + `Map<CharacterLayoutType, CharacterLayoutSettings>`, which holds `cardWidth`/`cardHeight`/`fontSize` **and** the photo box `imageWidth`/`imageHeight`; defaults: compact card 380×120 / image 140×108, portrait 380×160 / 150×148, gallery 380×220 / 380×188 with whole-image off). Dialog from the list app bar has **Card** (width/height/font) and **Image** (width/height) sections; persisted with `shared_preferences` per-type keys `charW_`/`charH_`/`charFont_`/`charImgW_`/`charImgH_`/`charWhole_`/`charLock_`; see `character_layout.dart` (`characterLayoutProvider`). The rendered card **and** photo box come from the pure function `characterCardMetrics(s, type, {maxWidth})`: whole-image (wholeImage || lockAspect) → the card grows to fit the photo box (compact/portrait width = `imageW + 8 gap + 120 textMin + 12 padding`, height = `imageH + 12`; gallery width = `max(cardWidth, imageW)`, height = `max(cardHeight, imageH + 32)`), but never wider than `maxWidth` (the list's available width, passed in by `_CharacterBody`'s `LayoutBuilder` — a `Wrap` clamps children to it, so the photo box shrinks instead of overflowing); crop mode → the card keeps its exact size and the photo is uniformly scaled to the leftover space. Cards are `SizedBox`es in a `Wrap`; photos use a fixed `SizedBox` (`compactPhotoBox`/`portraitPhotoBox`/`galleryPhotoBox` keys) with `CoverImage(fill: true, fillFit: showWholeImage ? BoxFit.contain : BoxFit.cover)` — no more `fixedHeight`. `lockAspect` links the **image** sliders to `layoutImageAspectProvider` (aspect of the first character cover photo, decoded via `instantiateImageCodec`; tests must override the provider — e.g. `overrideWith((ref) async => 2.0)` — since real file decoding never completes under fake async; falls back to `defaultAspect` 4:3). Compact card shows name (age) / profession / description lines.
