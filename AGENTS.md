...
- The character list has switchable card layouts (compact / portrait / gallery); each layout keeps its own width/height, font-size, whole-image and aspect-lock setting (`CharacterLayout` = selected type + `Map<CharacterLayoutType, CharacterLayoutSettings>`; defaults: compact 380×120, portrait 380×160, gallery 380×220 with whole-image off). Dialog from the list app bar, persisted with `shared_preferences` (per-type keys `charW_`/`charH_`/`charFont_`/`charWhole_`/`charLock_`); see `character_layout.dart` (`characterLayoutProvider`). Cards are laid out with a `Wrap` of fixed `SizedBox`es sized from `settings.cardWidth`/`cardHeight`. `showWholeImage` (wholeImage || lockAspect) true → whole photo shown never cropped (compact/portrait via `CoverImage(fixedHeight:)` aspect-contain, gallery via `CoverImage(fill: true, fillFit: contain)`); false → photo fills/crops the photo box (`CoverImage(fill: true)` cover, photo width = `cardWidth * 0.36`/`0.42`). `lockAspect` links the width/height sliders to `layoutImageAspectProvider` (aspect of the first character cover photo, decoded via `instantiateImageCodec`; tests must override the provider since real file decoding never completes under fake async — falls back to `defaultAspect` 4:3). Compact card shows name (age) / profession / description lines.
